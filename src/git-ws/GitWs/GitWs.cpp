...
namespace gitws
{
    ...
    void GitWs::initCmdPush()
    {
        ...
        flagAll.setBriefDesc(
            "Run the command in all repos (even non-ahead ones), for all "
            "branches.");
    ...
    void GitWs::initCmdSubmodule()
    {
        ...
        arg.setDesc(
            ...
            "recursively pulls the latest submodules from the remote, "
            "discarding any "
    ...
    void GitWs::initCmdDo()
    {
        ...
        flagAhead.setBriefDesc(
            "Run the command only in folders where repos are ahead of the "
            "remote?");
    ...
    void GitWs::initCmdQuery()
    {
        ...
        cmd.setDesc(
            "Queries the status of all the repos, returning whether they are "
            "changed "
            "or ahead.");

        auto& flagDisplayAll(cmd.create<Flag>("a", "display-all"));
        flagDisplayAll.setBriefDesc(
            "Display status of all repositories, even the ones without "
            "changes.");
